...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        """
        overplap = self.crossword.overlaps[x, y]
        words_to_remove = []
        if overplap == None:
            return False
        else:
            ...
            for word1 in self.domains[x]:
                overplap_possible = False
                for word2 in self.domains[y]:
                    if word1 != word2 and word1[i] == word2[j]:
                        overplap_possible = True
                        break
                if not overplap_possible:
    ...
    def ac3(self, arcs=None):
        ...
        if arcs is None:
            ...
            for variable1 in self.crossword.variables:
                for vairable2 in self.crossword.neighbors(variable1):
                    arcs.append((variable1, vairable2))
        for x, y in arcs:
            if self.revise(x, y):
